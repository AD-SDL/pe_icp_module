...
from metals_reference import metals_reference
...
class ICPInterface:
    ...
    exe_name = "Syngistix-ICP.exe"
    syngistix_pid = None
    """Process ID for the Syngistix .EXE"""
    syn_client = None
    """Syngistix remote client object"""
    ...
    samples = {}  # json object with pedigree
    json = None  # pathname for json file
    items = 90  # tubes in the rack
    columns = 15
    rows = 6

    # ICP attributes
    sample_info = f"rack{items}"
    methods = None  # the list of ICP methods
    last_method = "WASH"
    last_dataset = "Test"
    last_ID = last_dataset
    last_template = "MeansTemplate"

    # ICP analysis attributes
    num_analyzed = 0  # Number of samples that were analyzed
    analyzed = []  # list of samples done
    start_time = None
    completion_time = None
    elements = []  # elements to analyze
    autosampler = {}  # autosampler mappings
    Hg_on = False  # Hg alignment
    rinse = 1  # number of rinse cycles
    unexpected = True  # report "unexpected" samples
    decision = "Normal"  # high level decision to the module
    keep = False  # to keep plasma on after analysis
    last_error = None  # last error
    last_warning = {}  # last warning during analysis
    status = {}  # json status record
    wash = False  # if it is a wash cycle
    queue = []  # analsyis queue
    calibrate = True  # Whether to calibrate before and after analysis
    ...
    def __init__(
        ...
        self.exe_path = Path(dll_path).parent / self.exe_name
        self.server_ip = server_ip  # Syngistix remote server IP
        self.client_ip = client_ip  # IP of the client (this device)
    ...
    def to_json(self):
        self.status = {
            ...
            "done": self.num_analyzed,  # how many samples were analyzed
            "start": self.start_time,  # start datetime stamp of the analysis
            "finish": self.completion_time,  # finish datetime stamp of the analysis
    ...
    def restart_syngistix(self):
        try:
            proc = self.is_syngistix_running()
            if proc:
                proc.terminate()
                proc.wait()
    ...
    def is_syngistix_running(self):
        """Check to see if the syngistix executable is running and accessible."""
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                if self.exe_name in proc.info["name"]:
                    return proc
    ...
    def run_syngistix(self):
        """Start the Syngistix Executable, if it isn't running already"""
        for count in range(3):  # three attempts to restart
            try:
                self.syngistix_pid = self.is_syngistix_running()
                if self.syngistix_pid is None:
                    print("\n\n>> Attempt %d to (re)start Syngistix GUI" % (count + 1))

                    self.syngistix_pid = subprocess.Popen(
                        [str(self.exe_path)],
    ...
    def crash_plasma_off(
        self,
    ):  # if GUI crashes, try to restart GUI, optional plasma off, data storing
        self.decision = "Socket error"

        try:
            self.syngistix_pid.terminate()
            time.sleep(5)
        except Exception as e:
            ...
            try:
                if self.run_syngistix():
                    if self.connect2syngistix("Recovery run"):
                        ...
                        self.smtp.alert(
                            "Syngistix GUI recovered successfully"
                        )  # alerting
                        break
            except Exception as e:
                print(">> Crash recovery error: %s" % e)
    ...
    def connect2syngistix(self, name):
        """Connect to Syngistix using the RemoteSyngistix DLL"""
        if not os.path.exists(self.dll_path):
            print(">> Syngistix API folder %s missing, abort" % self.dll_path)
            return 0
        ...
        try:
            ...
            self.configure_callbacks(RemoteSyngistix)

            s = self.syn_client.Connect(
                clientIP=self.client_ip, serverIP=self.server_ip, name=name
            )
            if not s:
    ...
    def configure_callbacks(self, RemoteSyngistix):
        """Setup the various callbacks that the Syngistix DLLs use to communicate results"""
    ...
    def error_callback(
        ...
        self.instrument_error.append(
            {"timestamp": s, "severity": severity, "message": msg}
        )
    ...
    def start_auto_analysis(
        ...
        """Run an ICP auto analysis using the provided method"""
    ...
    def manual_sample(
        self, ID
    ):  # this is to run rinse manually to update sample info after <F2> stopping
    ...
    def stop(self, message, opt=1):  # soft stop: part of regular analysis polling
        ...
        self.completion_time = self.stamp()
        self.last_warning.update({self.completion_time: message})
    ...
    def hard_stop(
        self, message
    ):  # hard stop to terminate all activities, save files and plasma Off
    ...
    def plasma(self, state: bool = True):
        ...
        if state:
            ...
            while True:
                ...
                if self.verbose:
                    print(
                        "\n>> Attempt %d of %d to turn plasma on, 2 min wait"
                        % (count, max_retries)
                    )
                ...
                if self.plasma_status and "On" not in self.plasma_status:
                    print(
                        "\n>> Failed attempt %d of %d to turn plasma on"
                        % (count, max_retries)
                    )
                    if count == max_retries:
                        print(
                            "\n>> Cannot ignite plasma after %d repeated attempts, abort"
                            % max_retries
                        )
            ...
            if status != "Off":
                while True:
                    ...
                    if self.verbose:
                        print(
                            "\n>> Attempt %d of %d to turn plasma off, 30 s wait"
                            % (count, max_retries)
                        )
                    time.sleep(30)
                    if self.plasma_status and "Off" not in self.plasma_status:
                        print(
                            "\n>> Failed attempt %d of %d to turn plasma off"
                            % (count, max_retries)
                        )
                        if count == max_retries:
                            print(
                                "\n>> Cannot turn plasma off after %d repeated attempts, abort"
                                % max_retries
                            )
                            self.smtp.alert(
                                "Plasma Off failed, attend immediately",
                                importance="high",
                            )
                            return 0
                    else:
    ...
    def export(self, dataset: str, template: str):
        ...
        if self.syn_client:
            try:
                ...
                status = self.syn_client.Response()
                print(
                    ">> Export status %s for dataset %s with template %s"
    ...
    def sample2tube(self, i: int, size: int = 90):  # 60 or 90 tube rack wells
        if size == 90:
            cols = 15
        if size == 60:
            cols = 12
        ...
        with open(os.path.join(s, r), "w") as file:
            ...
            if mode:
                ...
                for i in range(self.items):
                    if i >= start:
                        ...
                        file.write(
                            "Data%d=%d,%d,%s,sample,%s\n"
                            % (d + 1, m + 1, i + 11, label, self.last_method)
                        )
                ...
                for i, label in self.autosampler.items():
                    if j >= start and j < size:
                        ...
                        if s not in self.analyzed:
                            ...
                            file.write(
                                "Data%d=%d,%d,%s,%s\n"
                                % (d + 1, m + 1, i + 10, label, self.last_method)
                            )
                            m += 1
                            d += 1
                    j += 1
            ...
            if mode == 0 and self.calibrate:
                file.write(
                    ...
                    % (m + 1, self.kind, self.last_method)
                )  # well 1
                m += 1
    ...
    def unpack_self_container(self, sort: bool = False):
        ...
        for ID in self.samples:
            ...
            self.autosampler[index] = "%s-%s-%s,%s" % (
                self.code,
                self.who,
                well_,
                self.kind,
            ...
            if self.verbose:
                print(
                    "%s %s -> ICP %s: sample %d (%s) :: %s"
                    % (self.who, well, tube, index, self.kind, s)
                )
        ...
        self.elements = sorted(self.elements, key=lambda x: metals_reference[x])
        if self.verbose:
    ...
    def convert_report(self, name):  # convert report to a better readable format
        ...
        if r:
            ...
            elements = sorted(
                df["Elem"].unique().tolist(), key=lambda x: metals_reference[x]
            )
            qs = []
            ...
            for ID in sample_IDs:
                ...
                for e in elements:
                    ...
                    for (
                        ...
                        subset.iterrows()
                    ):  # logs the last measurement for each sample ID and element
                        if row["Elem"] == e:
                            i = int(row["View"])
                            u = "%s_%s" % (views[i], e)
                            q[u] = row["Int (Corr)"]
    ...
    def check_run_completion(self):  # check completion of report after analysis
        flag = 0
        self.num_analyzed = self.count_completed()

        if self.num_analyzed < self.items:
            flag += 1
            if self.verbose:
                print(
                    ">> Incomplete analysis: %d samples out of %d"
                    % (self.num_analyzed, self.items)
                )
    ...
    def copy2exp(
        self, exp: str = None
    ):  # copy ICP result and status files to experiment
    ...
    def log_record(self):
        f = os.path.join(self.data_path, r"Reports\ICP_log.csv")
        try:
            if os.path.exists(f):
                out = open(f, "a")
            else:
    ...
    def poll_continuously(self, mode: int = 0):  # 1 when resume
        ...
        while True:
            ...
            if self.errors_after():
                if self.check_user_cancel:  # <F8> pause pressed
                    ...
                    self.smtp.alert(
                        "Errors/warnings during analysis", body=self.last_error
                    )
                    if self.is_syngistix_running():
                        flag = 5  # hard stop
                    else:
            ...
            if flag == 1:  # analysis in progress, count and log sample
                if s and s != self.last_sample:
                    sexp = self.queue[self.num_analyzed]
        ...
        if self.num_analyzed < len(self.queue):
            return 0, flag
    ...
    def run_wash(
        self, times=1
    ):  # special run for washing the lines that can be run at the end
        ...
        if self.start_auto_analysis(
            self.last_method, self.last_dataset, self.sample_info, self.last_template
        ):
            self.wait_analysis()
    ...
    def test_simple(self):
        ...
        self.write_sampleinfo(
            "test", 90
        )  # batch ID and the number of sample tubes in a rack
