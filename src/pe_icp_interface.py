...
class ICPInterface:
    ...
    def plasma_on(self, num_retrys = 5, retry_delay = 120):
      
            attempts = 0
            if not self.plasma_status:
                self.plasma_status = "Off"
            while attempts < num_retrys and "On" not in self.plasma_status:
                count += 1
                ...
                if self.plasma_status and "On" not in self.plasma_status:
                    ...
                    return True
            return False
    def plasma_off(self, num_retrys = 5, retry_delay = 120):
      
            attempts = 0
            if not self.plasma_status:
                self.plasma_status = "On"
            while attempts < num_retrys and "Off" not in self.plasma_status:
                count += 1
